...
class Student{
    ...
    void Test_Result() {
        ...
        bool user_Ntest = false; //don't have test and is a existing user // add record
        ...
        while (readexisting >> storedStudentID >> storedUsername_file >> storedScore >> storedStress_Level) {
            if (enterIdnum == storedStudentID) { //replace record
                ...
                //user_Ntest = true;
                userFound = true;
                cout << "have record and is a existing user" << endl;
                Enter();
                // Update the result
        ...
        ifstream readexisting_1 ("Std_testlevel.txt");

        if (!userFound){
            ofstream write_1("Std_testlevel.txt", ios::app);
            user_Ntest = true;
            cout << "no test and is a existing user" << endl;
            Enter();
            readexisting_1.close();
            if (write_1.is_open()){
                write_1 << enterIdnum << " " << storedUsername << " " << score << " " << stress_level << endl;
                write_1.close();
            }
        }
        ...
        if (!user_Ntest && !userExists) { //no test and is a new user
            ofstream write_2("Std_testlevel.txt", ios::app);
            cout << "no test and is a new user" << endl;
            Enter();
            if (write_2.is_open()){
                write_2 << Idnum << " " << name << " " << score << " " << stress_level << endl;
                write_2.close();
            }
        }
    }
